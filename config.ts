...
export const DEFAULT_MODES: Record<ModeName, ModeConfig> = {
	plan: {
		...
		systemPromptAddendum: [
			...
			"",
			"finalize_plan output — summary + body + steps + validation, plus optional docs and target_mode ('code'|'debug', default 'code'). Each field's own schema description is the source of truth for how to fill it; follow those. Two things to design TOWARD from Phase 1 on — they steer how you investigate and design, not just how you format the final call:",
			"- body must PIN the contracts at the seams — data shapes shared across components, API/file formats where two pieces must agree, cross-cutting decisions — while leaving internal function signatures to the implementer.",
			"- every validation entry must be a headless acceptance check the implementing agent can actually run (a command that exits, a `timeout`'d launch-and-kill smoke, a test harness, or a drivable scenario); mark a check only a human can confirm `(human)`.",
			"",
			"Constraints:",
