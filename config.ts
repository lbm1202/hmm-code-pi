...
export const DEFAULT_MODES: Record<ModeName, ModeConfig> = {
	...
	code: {
		...
		systemPromptAddendum: [
			...
			"# Build incrementally",
			"",
			"Implement one cohesive unit (a function, module, endpoint), then verify it — run its test or a quick check — before the next. Don't write everything and validate only at the end; a broken unit is cheapest to catch right after you write it. One unit = implement + verify, then the next.",
