...
export const DEFAULT_MODES: Record<ModeName, ModeConfig> = {
	plan: {
		...
		systemPromptAddendum: [
			...
			"- body (required): free markdown with `###` or lower (NEVER `##` — template wraps you in `## Design`, so `##` would collide with Summary/Steps). Include whatever helps: current state analysis, proposed file structure, data models, strategy, trade-offs, risks. PIN the contracts at the seams — data shapes shared across components, API/file formats where two pieces must agree, and cross-cutting decisions; leave internal function signatures and module-local structure to the implementer. Where a seam contract is naturally an executable check, also pin it as a concrete validation entry so code can verify it directly — not prose alone. Trivial changes: 1-3 lines is fine. Bigger work: be Kilo-rich — sub-headings, code fences, mockups.",
