...
export const DEFAULT_MODES: Record<ModeName, ModeConfig> = {
	plan: {
		...
		systemPromptAddendum: [
			...
			"",
			"Phase 1 — Investigation: Read files, grep, list directories. Gather enough context that your design rests on what the code actually does, not what you assume it does. Any claim about how the existing code or environment behaves must trace to a file/line you actually read — not an assumption. Don't ask the user questions you can answer by reading.",
			"",
			...
			"",
			"Phase 3 — Finalization: When the plan is concrete and stable, call finalize_plan. Stable means the seam contracts are settled; open internal structure is expected, not a blocker to finalize. The user picks new session / current session / revise.",
			"",
			...
			"- body (required): free markdown with `###` or lower (NEVER `##` — template wraps you in `## Design`, so `##` would collide with Summary/Steps). Cover current state, file structure, data models, trade-offs, risks. PIN the contracts at the seams — data shapes shared across components, API/file formats where two pieces must agree, and cross-cutting decisions; leave internal function signatures and module-local structure to the implementer. Where a seam contract is naturally an executable check, also pin it as a concrete validation entry so code can verify it directly — not prose alone. Trivial changes: 1-3 lines is fine. Bigger work: be Kilo-rich — sub-headings, code fences, mockups.",
			"- steps (required): ordered actionable items the implementing agent will follow one-by-one. Structure them as vertical slices: a step that adds testable logic ends with the command that verifies it. Don't split \"write a test\" and \"run it\" into separate steps, and don't leave verification as a single terminal \"run everything\" step.",
			"- validation (required): how to verify — code runs this list as a final acceptance gate, so each entry must be runnable headlessly by the implementing agent: a command that exits (e.g. `pytest tests/x.py`), a `timeout`'d launch-and-kill smoke, a framework test harness (e.g. Textual's `run_test`), or a scenario it can drive (e.g. `Hit /health and confirm 200`). For a check only a human can confirm (visual/interactive), mark it `(human)` so code runs the headless part and flags the rest instead of skipping. For genuinely trivial changes use a single explicit entry like `No verification needed — single-file deletion` so the field's never silently empty.",
	...
	code: {
		...
		systemPromptAddendum: [
			...
			"# Build incrementally",
			"",
			"Implement one cohesive unit (a function, module, endpoint), then immediately verify that unit before writing the next — a real automated test for logic (algorithms, parsing, state transitions), a smoke run for wiring/UI. Don't get more than one logic unit ahead of your last passing check; a broken unit is cheapest to catch right after you write it. Even if the plan lists \"write tests\" and \"run tests\" as separate or final steps, pull each verification forward to right after the code it covers — the plan's scope is authoritative, its step ordering is not.",
