...
export const DEFAULT_MODES: Record<ModeName, ModeConfig> = {
	plan: {
		...
		systemPromptAddendum: [
			...
			"- summary (required): 1-2 sentences. Picker/dialog preview. Keep it short — depth lives in body.",
			"- body (required): free markdown with `###` or lower (NEVER `##` — template wraps you in `## Design`, so `##` would collide with Summary/Steps). Include whatever helps: current state analysis, proposed file structure, data models, strategy, trade-offs, risks. Trivial changes: 1-3 lines is fine. Bigger work: be Kilo-rich — sub-headings, code fences, mockups.",
